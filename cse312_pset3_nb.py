...
of Washington CSE 312 for use solely during Spring Quarter 2023 for purposes
...
class NaiveBayes():
    ...
    of a set of emails that it hasn't seen before. See the slides and/or video for
    ...
    def fit(self, train_hams:List[str], train_spams:List[str]):
        ...
        def get_counts(filenames:List[str]) -> Dict[str, int]:
            ...
            """
            # size:int = len(filenames)
            dicc = {}
            for fil in filenames:
                zet = self.word_set(fil)
                # set_size:int = len(zet)
                for elem in zet:
                    if elem in dicc:
                        dicc[elem] += 1
                    else:
    ...
    def predict(self, filename:str) -> str:
        ...
        """
        # TODO: Your code here (~10-20 lines)
        pSpam = (self.num_train_spams)/(self.num_train_hams + self.num_train_spams)
        pHam = (self.num_train_hams)/(self.num_train_hams + self.num_train_spams)
        zet = self.word_set(filename)

        sumOfSpamWord = 0
        totalContS = 0
        for word in zet:
            totalContS = self.word_counts_spam.get(word, 0)
            wiCspam = (totalContS + 1)/(self.num_train_spams + 2)
            sumOfSpamWord += np.log(wiCspam)

        sumOfHamWord = 0
        totalContM = 0
        for word in zet:
            totalContM = self.word_counts_ham.get(word, 0)
            wiCham = (totalContM + 1)/(self.num_train_hams + 2)
            sumOfHamWord += np.log(wiCham)
        
        pSpam = np.log(pSpam)
        pHam = np.log(pHam)

        if ((pSpam + sumOfSpamWord) > (pHam + sumOfHamWord)):
